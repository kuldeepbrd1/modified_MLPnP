...
function [T, statistics] = MLPnP_mod(points3D, v, cov)
...
if planar
    ...
    scalefact0 = (abs(norm(P(:,1))*norm(P(:,2))*norm(P(:,3))))^(1/3);
    %(abs(norm(P(:,1))*norm(P(:,2))*norm(P(:,3))*norm(P(1,:))*norm(P(2,:))*norm(P(3,:))))^(1/6);
    %SVD to find the best rotation matrix in the Frobenius sense
    
    %% Mod - Kuldeep
    [U2,d2,V2] = svd(P(1:3,1:3)); 
    %correlation = P(1:3,1:3)'*P(1:3,1:3);
    scalefact = d2(2,2);
    %%
    R = U2*V2';
